...
from prepare_db import prepare_databases_for_metrics
from common import (
    init_logging_file,
    init_logging_stderr,
    init_logging_syslog,
...
DEFAULT_CONFIG_PATH = "postgresql-metrics.yml"
...
def main():
    ...
    for connection in db_connections:
        ...
        for m in metrics:
            s = 'INSERT INTO metrictable VALUES(' +"\'" +str(m['attributes']['what']) +"\'" + ', ' + str(m['value'])  +');'
            print(s)
            cursor.execute(s)
            if(m['attributes']['what'] == 'table-bloat'):
                s = 'INSERT INTO bloat VALUES(' + str(m['value'])  +');'
                cursor.execute(s)
            elif(m['attributes']['what'] == 'index-hit'):
                s = 'INSERT INTO indexhit VALUES(' + str(m['value'])  +');'
                cursor.execute(s)
            elif(m['attributes']['what'] == 'transaction-rate'):
                s = 'INSERT INTO transaction VALUES(' + str(m['value'])  +');'
                cursor.execute(s)
            elif(m['attributes']['what'] == 'transaction-rollbacks'):
                s = 'INSERT INTO transactionroll VALUES(' + str(m['value'])  +');'
                cursor.execute(s)
        connection.close()
    return metrics
...
#    metricvalue INT 
# );
# delete from metrictable;
# delete from bloat;
# delete from indexhit;
# delete from transaction;
# delete from transactionroll;

# Create table bloat (floatn float4 not null);
# insert into bloat (floatn) values (1.23);
# select * from bloat;
# ALTER TABLE bloat ADD COLUMN time TIMESTAMP DEFAULT NOW();
# Create table indexhit (floatn float4 not null);
# insert into indexhit (floatn) values (1.23);
# select * from indexhit;
# ALTER TABLE indexhit ADD COLUMN time TIMESTAMP DEFAULT NOW();

# CREATE TABLE transactionroll (
#    metricvalue INT 
# );
# ALTER TABLE transactionroll ADD COLUMN time TIMESTAMP DEFAULT NOW();